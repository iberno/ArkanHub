...
async function main() {
    ...
    for (const c of categorySeed) {
        ...
        if (!cat)
            cat = await prisma.ticketCategory.create({ data: c });
    ...
    for (const key of permData.map(p => p.key)) {
        ...
        if (!exists)
            await prisma.rolePermission.create({ data: { roleId: roleIds['admin'], permissionId: permIds[key] } });
    }
    const rolePermMap = {
    ...
    for (const [roleName, perms] of Object.entries(rolePermMap)) {
        for (const key of perms) {
            ...
            if (!exists)
                await prisma.rolePermission.create({ data: { roleId: roleIds[roleName], permissionId: permIds[key] } });
        }
    }
    const companyData = [
    ...
    for (const u of userDefs) {
        if (u.isManager)
            managerMap[u.dept + '|' + u.company] = userIds[u.email];
    }
    ...
    for (const w of workflowDefs) {
        ...
        for (const cond of w.conditions) {
            ...
            if (!exists)
                await prisma.workflowCondition.create({ data: { workflowId: rule.id, ...cond } });
        }
        for (const act of w.actions) {
            ...
            if (!exists)
                await prisma.workflowAction.create({ data: { workflowId: rule.id, actionType: act.type, payload: act.payload } });
        }
    }
    const articleDefs = [
        {
    ...
    for (const name of knowledgeCatNames) {
        ...
        if (!cat)
            cat = await prisma.knowledgeCategory.create({ data: { name } });
