...
async function main() {
    ...
    const assignees = arkanhubUsers.filter(u => u.roles.some(r => assignableRoles.has(r.role.name)));
    const nonArkanhub = users.filter(u => u.companyId !== arkanhubId);
    const arkanhubRequesters = arkanhubUsers.filter(u => u.roles.some(r => r.role.name === 'admin' || r.role.name === 'gestor_ti'));
    ...
    for (let i = 0; i < TOTAL; i++) {
        ...
        let slaId = slaCorp;
        if (priorityName === 'Crítica')
            slaId = slaP1;
        else if (categoryId === catByName['Acessos'])
            slaId = slaAcessos;
        else if (categoryId === catByName['Projetos'])
            slaId = slaProjetos;
        else if (requester.email === 'admin@arkanhub.com' || requester.email === 'carlos@arkanhub.com')
            slaId = slaVip;
        let resolvedAt = null;
        let closedAt = null;
        if (isResolved) {
...
//# sourceMappingURL=seed-1200.js.map